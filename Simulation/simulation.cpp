...
=======
  size_t position_of_dying_cell = death_prob_rnd[parent].draw_explicit(rndgen);
>>>>>>> Stashed changes
...
=======
  size_t position_of_grown_cell = growth_prob_rnd[parent].draw_explicit(rndgen);
>>>>>>> Stashed changes
...
=======
      growth_prob_rnd[i].update_entry(pos, probs[i]);
  }
}
...
void simulation::update_death_prob(size_t pos) {
    for (size_t i = 0; i < 4; ++i) {
        ...
        death_prob_rnd[i].update_entry(pos, new_val);
    }
>>>>>>> Stashed changes
}
...
simulation::simulation(const Param& param) {
  ...
  std::vector<float> filler(world.size(), 0.f);
  for(int i = 0; i < 4; ++i) {
    growth_prob_rnd[i] = binned_distribution(filler.begin(), filler.end(), sq_size);
    death_prob_rnd[i]  = binned_distribution(filler.begin(), filler.end(), sq_size);
  }
...
=======
    growth_prob_rnd[normal].update_entry(pos, probs[normal]);
    growth_prob_rnd[cancer].update_entry(pos, probs[cancer]);
    growth_prob_rnd[infected].update_entry(pos, probs[infected]);
    growth_prob_rnd[resistant].update_entry(pos, probs[resistant]);

    death_prob_rnd[i.node_type].update_entry(pos, 1.f);
>>>>>>> Stashed changes
  }
}
...
std::vector<int> simulation::get_cell_numbers() {
    ...
    output[normal]      =    static_cast<int>(death_prob_rnd[normal].get_total_sum());
    output[cancer]      =    static_cast<int>(death_prob_rnd[cancer].get_total_sum());
    output[infected]    =    static_cast<int>(death_prob_rnd[infected].get_total_sum());
    output[resistant]   =    static_cast<int>(death_prob_rnd[resistant].get_total_sum());
    return output;
}
