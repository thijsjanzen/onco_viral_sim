...
//
cinekine::voronoi::Graph build(cinekine::voronoi::Sites&& sites, float xBound, float yBound)
{
    cinekine::voronoi::Graph graph(xBound, yBound, std::move(sites));

    cinekine::voronoi::Sites& graphSites = graph._sites;
    ...
    const cinekine::voronoi::Site* siteData = graphSites.data();
    const cinekine::voronoi::Site* lastSiteData = nullptr;
    ...
    {
        const cinekine::voronoi::Site& r1 = graphSites[site1];
        const cinekine::voronoi::Site& r2 = graphSites[site2];
        if (r2.y > r1.y)
            return true;
        if (r2.y < r1.y)
            return false;
    ...
    cinekine::voronoi::Cells& cells = graph._cells;
    cells.reserve(siteEvents.size());

    cinekine::voronoi::Fortune fortune(graph);
    ...
    while(1)
    {
        ...
        cinekine::voronoi::Site* site = siteIndex >= 0 ? &graphSites[siteIndex] : nullptr;
